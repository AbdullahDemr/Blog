$(document).ready(function () {
    ...
    const dataTable = $('#deletedPatientsTable').DataTable({
        dom:
        ...
        buttons: [
            {
                ...
                action: function (e, dt, node, config) {
                    $.ajax({
                        type: 'GET',
                        url: '/Admin/Patient/GetAllDeletedCategories/',
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            let patientName = tableRow.find('td:eq(1)').text();

            Swal.fire({
                title: 'Arşivden geri getirmek emin misiniz?',
                text: `${patientName} adlı kategori arşivden geri getirilecektir!`,
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        dataType: 'json',
                        data: { patientId: id },
                        url: '/Admin/Patient/UndoDelete/',
                        success: function (data) {
                            const undoDeletedPatientResult = jQuery.parseJSON(data);
                            if (undoDeletedPatientResult.ResultStatus === 0) {
                                Swal.fire(
                                    'Arşivden Geri Getirildi!',
                                    `${undoDeletedPatientResult.Message}`,
                                    'success'
                                );
                                ...
                                Swal.fire({
                                    ...
                                    text: `${undoDeletedPatientResult.Message}`,
                                });
                            }
                        },
                        error: function (err) {
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            Swal.fire({
                title: 'Kalıcı olarak silmek istediğinizden emin misiniz?',
                text: `${patientName} adlı kategori kalıcı olarak silinecektir!`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
