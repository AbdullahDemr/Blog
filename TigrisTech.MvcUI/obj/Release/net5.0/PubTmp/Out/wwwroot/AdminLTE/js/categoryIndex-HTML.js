$(document).ready(function () {
    ...
    $(function () {
        ...
        /* Ajax GET / Getting the _CategoryAddPartial as Modal From ends here. */
        ...
        placeHolderDiv.on('click', '#btnSave', function (event) {
            ...
            $.post(actionUrl, dataToSend).done(function (data) {
                const categoryAddjaxModel = jQuery.parseJSON(data);

                const newFormBody = $('.modal-body', categoryAddjaxModel.CategoryAddPartial);
                ...
                if (isValid) {
                    ...
                    const newTableRow =
                        `
                        <tr name="${categoryAddjaxModel.CategoryDto.Category.Id}">
                            <td>${categoryAddjaxModel.CategoryDto.Category.Id}</td>
                            <td>${categoryAddjaxModel.CategoryDto.Category.Name}</td>
                            <td>${categoryAddjaxModel.CategoryDto.Category.Description}</td>
                            <td>${categoryAddjaxModel.CategoryDto.Category.IsActive ? "Evet" : "Hayır"}</td>
                            <td>${categoryAddjaxModel.CategoryDto.Category.IsDeteted ? "Evet" : "Hayır"}</td>
                            <td>${categoryAddjaxModel.CategoryDto.Category.Note}</td>
                            <td>${categoryAddjaxModel.CategoryDto.Category.CreateDate}</td>
                            <td>${categoryAddjaxModel.CategoryDto.Category.ModifiedByName}</td>
                            <td>${categoryAddjaxModel.CategoryDto.Category.CreateByName}</td>
                            <td>${categoryAddjaxModel.CategoryDto.Category.ModifiedDate}</td>
                             <td>
                                <button class="btn btn-primary btn-sm btn-update" data-id="${categoryAddjaxModel.CategoryDto.Category.Id}"><span class="fas fa-edit"></span></button>
                                <button class="btn btn-danger btn-sm btn-delete" data-id="${categoryAddjaxModel.CategoryDto.Category.Id}"><span class="fas fa-minus-circle"></span></button>
                            </td>
                        </tr>
                        `;
                    ...
                    toastr.success(`${categoryAddjaxModel.CategoryDto.Message}`, 'Başarılı İşlem!');
                }
                else {
                    let summaryText = "";
    ...
    $(document).on('click', '.btn-delete', function (event) {
        ...
        const categoryName = tableRow.find('td:eq(1)').text();//1 index Name 
        Swal.fire({
